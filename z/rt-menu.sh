...
# In strict POSIX sh, simple 'set -e' might be used, but this script uses specific logic.
...
# Function to create custom LuCI menu entries
create_luci_menu() {
    log_info "Creating LuCI menu entries..."

    # 1. Get Router IP for iframe links
    local lan_ip=$(uci -q get network.lan.ipaddr || echo "192.168.1.1")

    # 2. Create Controller for Zapret-Manager Terminal
    cat <<EOF > /usr/lib/lua/luci/controller/zms_terminal.lua
module("luci.controller.zms_terminal", package.seeall)

function index()
    -- Add item to menu: Services -> Zapret Terminal
    entry({"admin", "services", "zms_terminal"}, template("zms_terminal"), "Zapret Terminal", 90).leaf = true
end
EOF

    # 3. Create View (HTML) for Zapret-Manager (iframe on port 7681)
    mkdir -p /usr/lib/lua/luci/view
    cat <<EOF > /usr/lib/lua/luci/view/zms_terminal.htm
<%+header%>
<h2 name="content">Zapret Manager Terminal</h2>
<div class="cbi-map">
    <div class="cbi-section">
        <script type="text/javascript">
            // Dynamically construct URL using current protocol and hostname
            var url = window.location.protocol + "//" + window.location.hostname + ":7681";
            document.write('<iframe src="' + url + '" style="width: 100%; height: 600px; border: none;"></iframe>');
        </script>
    </div>
</div>
<%+footer%>
EOF
    ...
    # 4. Create Controller for OpenWrt Shell Terminal
    cat <<EOF > /usr/lib/lua/luci/controller/shell_terminal.lua
module("luci.controller.shell_terminal", package.seeall)

function index()
    -- Add item to menu: Services -> Shell Terminal
    entry({"admin", "services", "shell_terminal"}, template("shell_terminal"), "OpenWrt Shell", 91).leaf = true
end
EOF

    # 5. Create View (HTML) for Shell (iframe on port 7682)
    cat <<EOF > /usr/lib/lua/luci/view/shell_terminal.htm
<%+header%>
<h2 name="content">OpenWrt Shell Terminal</h2>
<div class="cbi-map">
    <div class="cbi-section">
        <script type="text/javascript">
            var url = window.location.protocol + "//" + window.location.hostname + ":7682";
            document.write('<iframe src="' + url + '" style="width: 100%; height: 600px; border: none;"></iframe>');
        </script>
    </div>
</div>
...
EOF
    ...
    # 6. Clear LuCI cache to apply menu changes
    rm -rf /tmp/luci-indexcache
    rm -rf /tmp/luci-modulecache/
    
    log_success "LuCI menu entries created!"
}
...
toggle_web_access() {
    ...
    if [ "$is_enabled" -eq 1 ]; then
        ...
        # === Remove LuCI Menu Items ===
        rm -f /usr/lib/lua/luci/controller/zms_terminal.lua
        rm -f /usr/lib/lua/luci/view/zms_terminal.htm
        rm -f /usr/lib/lua/luci/controller/shell_terminal.lua
        rm -f /usr/lib/lua/luci/view/shell_terminal.htm
        ...
        log_success "Web access and menu items removed successfully!"

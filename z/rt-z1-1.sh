...
zapret_update_mode() {
    ...
    if opkg list-installed | grep -q zapret; then
        ...
        echo "ℹ️  ZAPRET is not installed. Skipping update."
    fi
    
    echo ""
    echo "✅ ZAPRET update completed successfully!......."
    echo ""
}
...
instagram_dns_mode() {
    ...
    if [ -d "/opt/zapret" ]; then
        ...
        echo "ℹ️  ZAPRET directory not found. Please install ZAPRET first."
        exit 1
    fi
...
echo "3) ZAPRET update only"
echo "4) Instagram DNS update only"
echo "5) Complete ZAPRET update (ZAPRET + DNS)"
echo ""
echo "Notes:"
echo "- For Clean install, you can provide WiFi parameters as arguments:"
echo "  $0 2 \"WiFi5_SSID\" \"WiFi2.4_SSID\" \"Password\""
echo "- Run mode 1 first to update all packages"

...
# Check if backup exists and restore it (used in modes 1 and 3)
check_and_restore_backup() {
    if [ -f "$BACKUP_FILE" ]; then
        log_info "Backup file detected at $BACKUP_FILE. Restoring network settings..."
        restore_config
    else
        log_info "No backup file found at $BACKUP_FILE. Continuing without restoring."
    fi
}

# Full automatic install mode (previously option 7)
...
# Full automatic update mode (previously option 1)
update_mode() {
...
# Combined ZAPRET and Instagram DNS update/install mode (formerly 4,5,6)
zapret_dns_mode() {
...
# Factory reset with restore (modified option 7)
